...
function createProductElement(product){
    ...
    <div class="flex bg-gray-100 justify-center relative overflow-hidden group cursor-pointer border rounded-xl">
...
function filterProducts(){
    ...
    productElements.forEach((productElement, index)=>{
        ...
        // check to see if the product mathces the search or the checked categories
        const matchesSearchTerm = product.name.toLowerCase().includes(searchTerm);
        const isInCheckedCategory = checkedCategories.length === 0 || checkedCategories.includes(product.category);

        // show or hide the product based on the search and checked categories
